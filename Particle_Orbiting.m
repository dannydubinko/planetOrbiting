...
%% System Properties and Initial codition - DO NOT EDIT IN PART 1
...
%examine initial state (use plot3. Make the particles different colors and
%the center of mass black) Have your TA check you center of mass expression
%as well as your plot before moving on.

figure
hold on;
plot3(p1(1,1), p1(1,2), p1(1,3),'.r', 'MarkerSize', 25);
plot3(p2(1,1), p2(1,2), p2(1,3),'.g', 'MarkerSize', 25);
plot3(p3(1,1), p3(1,2), p3(1,3),'.b', 'MarkerSize', 25);
plot3(p4(1,1), p4(1,2), p4(1,3),'.m', 'MarkerSize', 25);
...
lgd = legend('particle 1', 'particel 2', 'particle 3', 'particle 4', 'Center of Mass');
...
figure
hold on;
plot3(p1_out(2:end,1), p1_out(2:end,2), p1_out(2:end,3),'r','LineWidth', 2);
...
plot3(com_out(2:end,1), com_out(2:end,2), com_out(2:end,3),'k','LineWidth', 2);
view(45,45);
...
lgd = legend('Trajectory 1', 'Trajectory 2', 'Trajectory 3', 'Trajectory 4', 'Center of Mass');
...
for i = 2:50:length(p1_out)
    ...
    plot3(com_out(i,1), com_out(i,2), com_out(i,3),'.k','markersize', 13);
    drawnow
    xlabel("X-Trajectory[m]");
    ylabel("Y-Trajectory[m]");
    zlabel("Z-Trajectory[m]");
...
%Linear Momentum of Entire System
mv_system = mv1_out + mv2_out + mv3_out + mv4_out;

figure
hold on;
plot(time(:,1),mv_system(:,1),'r','LineWidth', 2);
plot(time(:,1),mv_system(:,2),'g','LineWidth', 2);
plot(time(:,1),mv_system(:,3),'b','LineWidth', 2);
ylim([-1,1]);
ylabel("Momentum of System [kgm/s]");
xlabel("Time [s]");
title('Linear Momentum of Sytem vs. Time');
lgd = legend('X - Direction', 'Y - Direction', 'Z - Direction');
title(lgd,'System Momentum','FontSize',10);
hold off;

%% Angular momentum  
%angular momentum of Particle 1 in x,y,z directions

ang_momentum1 = cross(p1_out, mv1_out);
ang_momentum2 = cross(p2_out, mv2_out);
ang_momentum3 = cross(p3_out, mv3_out);
ang_momentum4 = cross(p4_out, mv4_out);

figure
hold on;
plot(time(:,1),ang_momentum1(:,1),'r','LineWidth', 2);
plot(time(:,1),ang_momentum1(:,2),'g','LineWidth', 2);
plot(time(:,1),ang_momentum1(:,3),'b','LineWidth', 2);
ylabel("Angular Momentum [kgm/s]");
xlabel("Time [s]");
title('Angular Momentum - Particle 1 vs. Time');
lgd = legend('X - Direction', 'Y - Direction', 'Z - Direction');
title(lgd,'Angular Momentum','FontSize',10);
hold off;
%angular momentum of Particle 2 in x,y,z directions
figure
hold on;
plot(time(:,1),ang_momentum2(:,1),'r','LineWidth', 2);
plot(time(:,1),ang_momentum2(:,2),'g','LineWidth', 2);
plot(time(:,1),ang_momentum2(:,3),'b','LineWidth', 2);
ylabel("Angular Momentum [kgm/s]");
xlabel("Time [s]");
title('Angular Momentum - Particle 2 vs. Time');
lgd = legend('X - Direction', 'Y - Direction', 'Z - Direction');
title(lgd,'Angular Momentum','FontSize',10);
hold off;
%angular momentum of Particle 3 in x,y,z directions
figure
hold on;
plot(time(:,1),ang_momentum3(:,1),'r','LineWidth', 2);
plot(time(:,1),ang_momentum3(:,2),'g','LineWidth', 2);
plot(time(:,1),ang_momentum3(:,3),'b','LineWidth', 2);
ylabel("Angular Momentum [kgm/s]");
xlabel("Time [s]");
title('Angular Momentum - Particle 3 vs. Time');
lgd = legend('X - Direction', 'Y - Direction', 'Z - Direction');
title(lgd,'Angular Momentum','FontSize',10);
hold off;
%angular momentum of Particle 4 in x,y,z directions
figure
hold on;
plot(time(:,1),ang_momentum4(:,1),'r','LineWidth', 2);
plot(time(:,1),ang_momentum4(:,2),'g','LineWidth', 2);
plot(time(:,1),ang_momentum4(:,3),'b','LineWidth', 2);
ylabel("Angular Momentum [kgm/s]");
xlabel("Time [s]");
title('Angular Momentum - Particle 4 vs. Time');
lgd = legend('X - Direction', 'Y - Direction', 'Z - Direction');
title(lgd,'Angular Momentum','FontSize',10);
hold off;

%angular momentum of a the system in x,y,z directions
p_out = p1_out + p2_out + p3_out + p4_out;
ang_momentum_system = cross(p_out, mv_system);

figure
hold on;
plot(time(:,1),ang_momentum_system(:,1),'r','LineWidth', 2);
plot(time(:,1),ang_momentum_system(:,2),'g','LineWidth', 2);
plot(time(:,1),ang_momentum_system(:,3),'b','LineWidth', 2);
...
%% Net Force Function
%Here we are going to create a function that will take in all the masses
